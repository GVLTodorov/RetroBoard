// Browser-driven demo recorder: drives 5 real Chromium sessions through the actual
// Home/Join/Board UI (one facilitator + 4 guests) -- everyone joins, writes cards (blur-until-
// reveal is on, so the facilitator reveals each column), votes, the facilitator ends voting and
// advances to action items, then converts the top card. Only the facilitator's session is
// recorded to video. Every wait targets a specific element (never NetworkIdle -- SignalR's
// persistent connection means that would never settle), so a stuck flow surfaces as a Playwright
// TimeoutException and a non-zero exit code: this doubles as a live smoke test of
// join/write/reveal/vote/advance, not just a video-generation script.
//
// Unlike PlanningPoker.Tests.Play, this needs no external API key -- RetroBoard has no optional
// Giphy avatar integration to skip (REQUIREMENTS.MD Section 4.1: zero required configuration).
//
//   dotnet run --project RetroBoard.Tests.Play -c Release -- http://localhost:6233 artifacts/demo-video
...
string[] participantNames = ["Jordan Lee", "Emma Novak", "Diego Fernández", "Aisha Khan", "Liam O'Connor"];
...
// facilitator", and keeping the other 4 sessions unrecorded keeps the artifact small and focused.
...
// participant card appearing individually rather than all four popping in at once.
...
static int rngDelay() => Random.Shared.Next(400, 900);
